...
def card_recommend_tool(budget: Optional[int|str], categories: List[str], search_query: str) -> str:
    ...
    # file_path = os.path.join(current_dir, "dummy_RAG_output_Shinhan.json")
    
    # print(f"[DEBUG] 파일 찾기 시도 경로: {file_path}\n") # 터미널에서 경로 확인용
    
    # try:
    #     with open(file_path, "r", encoding="utf-8") as j:
    #         dummy_data = json.load(j)
    # except FileNotFoundError:
    #     print(f"[ERROR] 🚨 파일을 찾을 수 없습니다: {file_path}")
    #     # 에러 메시지를 명확하게 담아 반환
    #     dummy_data = {"error": f"파일을 찾을 수 없습니다. 경로: {file_path}"}
...
def call_search_tool_node(state: AgentState):
    ...
    # Tool 직접 호출
    raw_search_result = card_recommend_tool.invoke({
        "budget": analysis.budget, 
        "categories": analysis.categories,
        "search_query": analysis.search_query + category_summary
    })
    
    try:
        # 1. 원본 데이터 로드
        raw_data_sources = json.loads(raw_search_result)
        
        if not raw_data_sources or "error" in raw_data_sources:
            return {"messages": [AIMessage(content="죄송합니다. 조건에 맞는 카드를 찾지 못했습니다.")]}

        # 2. [핵심] card_name 기준으로 Data Aggregation (다중 카드 지원)
        grouped_cards = {}
        # [변경점] 2중 루프: 소스(Source) -> 청크(Chunk) 순회
        for source_name, chunks in raw_data_sources.items():
            # chunks가 딕셔너리가 아닌 경우(에러 메시지 등) 스킵
            if not isinstance(chunks, dict):
                continue
                
            for chunk_id, chunk in chunks.items():
                c_name = chunk.get("card_name")
                # 불량 데이터(이름이 없는 청크) 스킵
                if not c_name:
                    continue
                    
                if c_name not in grouped_cards:
                    # 새 카드가 발견되면 기본 뼈대 생성
                    grouped_cards[c_name] = {
                        "card_name": c_name,
                        "card_company": chunk.get("card_company", "확인불가"),
                        "annual_fee": chunk.get("annual_fee", 0),
                        "min_performance": chunk.get("min_performance", 0),
                        "major_categories": chunk.get("major_categories", []),
                        "benefits_summary": chunk.get("benefits_summary", ""),
                        "detailed_benefits": []
                    }
                
                # 해당 카드의 혜택 리스트에 청크의 상세 정보 누적
                grouped_cards[c_name]["detailed_benefits"].append({
                    "category": chunk.get("category", ""),
                    "content": chunk.get("content", ""),
                    "conditions": chunk.get("conditions", "")
                })        
        # for key, chunk in raw_chunks.items():
        #     c_name = chunk.get("card_name")
        #     # 불량 데이터(이름이 없는 청크) 스킵
        #     if not c_name:
        #         continue
...
{custom_benefit}"""
        ...
        refined_data = raw_search_result
...
#         # 3. 단일 LLM 호출로 전체 카드의 맞춤 혜택 요약 (지연 시간 방지)
                    
#         # print(f"[DEBUG] 카테고리 상세 설명 정보: \n{category_summary}")
...
#         위 데이터를 바탕으로 각 카드별로 유저 주요 지출 분야와 일치하는 핵심 혜택을 2~3개씩 추출해 불릿 포인트('- ')로 요약해줘.
#         반드시 [카드명]을 말머리로 달아서 구분되게 출력하고, 불필요한 인사말은 생략해.
#         """
#         llm_benefits_summary = llm.invoke([SystemMessage(content=benefit_prompt)]).content
        
#         # 4. 결정론적 데이터 계산 및 텍스트 포맷팅 (타입 에러 방어 적용)
#         response_parts = []
#         user_budget = analysis.budget
        
#         for c_name, card_info in grouped_cards.items():
#             # [방어 로직] 문자가 섞여 들어올 경우를 대비한 캐스팅
#             try:
#                 min_perf = int(card_info.get("min_performance", 0))
#                 annual_fee = int(card_info.get("annual_fee", 0))
#             except (ValueError, TypeError):
...
#                 budget_status = "(예산 상관없음)"
#             elif isinstance(user_budget, int):
#                 budget_status = "✅ (실적 충족 가능)" if user_budget >= min_perf else "⚠️ (실적 미달 주의)"
#             else:
#                 budget_status = ""

#             card_header = f"""[추천 카드 정보]
# 💳 카드명: {c_name} ({card_info.get('card_company')})
# 💰 연회비: {annual_fee:,}원
# 📊 필요 실적: {min_perf:,}원 {budget_status}
# 💡 핵심 요약: {card_info.get('benefits_summary')}"""
            
#             response_parts.append(card_header)

#         # 5. 최종 메시지 결합
#         final_response = "\n\n".join(response_parts)
#         final_response += f"\n\n[고객님 맞춤 혜택 상세]\n{llm_benefits_summary}\n\n💬 더 궁금한 혜택이나 상세 조건이 있다면 편하게 물어봐 주세요!"
        
#         # QA 노드에서 활용하기 위해 그룹화된 리스트 자체를 저장
#         refined_data = list(grouped_cards.values())
