"""
test_extract_doctor.py

n_extract_doctor 단독 테스트 — xlsx에서 특정 환자의 인터뷰를 읽어
의사 발언 추출 결과(doctor_text)를 확인한다.

실행 방법:
    cd C:/Users/yebin/Desktop/Medical
    python tests/test_extract_doctor.py --patient p01
"""
...
from medical_workflow.nodes.extraction import n_extract_doctor
...
def run_case(llm, transcript: str, label: str = "") -> None:
    ...
    result = n_extract_doctor(s, llm)

    print("\n[원본 transcript]")
    print(transcript)

    print("\n[추출된 doctor_text]")
    print(result.get("doctor_text", "(없음)"))
